...
public:
    ...
    * allocates dynamic memory. This deends on your implementation
...
private:
   ...
   * You may want a recursive helper function for this one.
    */
   void clear();
   void renderHelper(PNG * mypng, Node* currentNode);
   void clearHelper(Node* currentNode);
   Node* copyHelper(Node* otherNode);
   void pruneHelperForTraversing(Node* currNode, double pct, int tol);
   bool pruneHelperForTolerance(Node* topRoot, Node* currNode, int tol);
   int pruneHelperForIndividualNode(Node* topRoot, Node* currNode, int tol);
