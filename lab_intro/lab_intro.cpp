...
PNG createSpotlight(PNG image, int centerX, int centerY) {
  for(unsigned x =0; x<image.width(); x++){
    for(unsigned y=0; y<image.height(); y++){
      HSLAPixel *pixel = image.getPixel(x,y);
      unsigned xAway = x - centerX;
      unsigned yAway = y - centerY;
      double distanceaway = sqrt(xAway*xAway + yAway*yAway);
      if(distanceaway > 200){
        pixel->l =0;
      }else{
      pixel->l = pixel->l*(1-((distanceaway*0.5)/100));
      }
...
PNG ubcify(PNG image) {
  for(unsigned x=0; x<image.width(); x++){
    for(unsigned y=0; y<image.height(); y++){
      HSLAPixel *pixel = image.getPixel(x,y);
      double hueAtPixel = pixel->h;
      if(fabs(210-hueAtPixel)>fabs(40-hueAtPixel)){
        pixel->h=40;
      }else{
        pixel->h=210;
      }
