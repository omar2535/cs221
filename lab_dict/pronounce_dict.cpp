...
bool PronounceDict::homophones(const string& word1, const string& word2) const
{
    string wd1 = word1;
    string wd2 = word2;
    std::transform(wd1.begin(), wd1.end(), wd1.begin(), ::toupper);
    std::transform(wd2.begin(), wd2.end(), wd2.begin(), ::toupper);

    vector<std::string> pronv1;
    vector<std::string> pronv2;
    auto pron1 = dict.find(wd1);
    if(pron1 != dict.end()){
        pronv1 = pron1->second;
    }else{
        return false;
    }
    auto pron2 = dict.find(wd2);
