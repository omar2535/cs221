...
void AVLTree<K, V>::rebalance(Node*& subtree)
{
    ...
    if(height(subtree->left) - height(subtree->right) > 1 && height(subtree->left->left) - height(subtree->left->right) < 0){
        rotateLeftRight(subtree);
    }
     if(height(subtree->right) - height(subtree->left) > 1  && height(subtree->right->right) - height(subtree->right->left) < 0){
